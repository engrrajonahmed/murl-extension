...
let getrandom = () => {
	let text = ''
	let possible = 'abcdefghjkmnopqrstuvwxyz'
	for (let i = 0; i < 5; i++) {
...
let send_request = (url) => {
	...
	shortenedURL.value = `https://murl.ml/#${hash}`
	copyer('shortenedURL')
	sucess.innerHTML = 'Copy shortened URL to clipboard 🚀'
	...
	if (!protocol_ok) {
		...
		if (custominput.value == '') {
			...
			if (cre.test(custominput.value)) {
				...
				erbox.innerHTML = '😮 Invalid custom alias, use only alphabets'
				custominput.placeholder = custominput.value
				custominput.value = ''
				rotate.classList.remove('spinning')
				rotate.style.display = 'none'
